      ...
      ++it)
  {
    ...
    function = 
      entry_obj.find("function")
        ->second
        .value;  // we assume that all entries in the json file are located in the same function
    // insert the entity
    spec.insert_entity(
    ...
    spec.set_minus_func(to_json_object(entry_obj.find("abst-functions")->second)
                          .find("sub-abs-conc")->second.value);
...
abstraction_spect::get_abstraction_function_files() const
{
  ...
  std::unordered_map<irep_idt, irep_idt> _name_pairs) const
{
  ...
  {
    throw "old abstraction_spect should match the callee";
  }
...
std::vector<exprt> 
abstraction_spect::spect::get_assumption_exprs(const namespacet &ns) const
{
...
std::vector<exprt> abstraction_spect::spect::abst_shapet::get_assumption_exprs(
  const namespacet &ns,
